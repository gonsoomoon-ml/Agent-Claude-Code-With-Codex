...
PROMPT_VERSION = "represent-v3.2"  # v3.2: v3.1(선택 규칙·논조 보존·예산 3~5) + 수치는 조건과 한 몸
# (v3.1 이 블라인드 심사 충실도에서 v3 에 진 유일한 이유 = 조건절 생략: "2.4/6.4배(커널만/커널+브로드캐스팅)",
#  "8배(밀집 대비)" 의 괄호를 떨궈 없던 보편성·인과가 생겼다. 조건절은 10~30자라 예산을 안 늘린다.)
...
_OUTPUT_CONTRACT = (
    ...
    "한 절이라도 포함. 원문의 귀속(누가 주장했나)·유보 표현과 **수치의 측정 조건**을 유지"
