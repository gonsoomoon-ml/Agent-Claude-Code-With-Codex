...
- **빌드/실행 (UV):** `uv sync` · `uv run ruff check src tests`(린트) · `uv run pytest`(테스트 **42개** — 불변식·결정론, DI fake 로 전 파이프라인) · `uv run python -m briefing.local.run`(AWS-free baseline) · **실 e2e:** `[DEBUG=1] uv run python scripts/e2e_smoke.py`(실 RSS→`claude -p` author→`codex` certifier→render — 라이브 검증). **구조:** `src/briefing/` = `shared/`(진실: config·sources·source_store·lenses·author·gate·certifier·render·**curation·pipeline·_debug**·prompts) / `runtime/`(AgentCore 얇은 어댑터 + Strands **supervisor** 옵션) / `local/`(AWS-free baseline). **하니스:** author=`claude -p`·certifier=`codex exec` — 둘 다 clean dir·subprocess·cross-family. **수동 author:** `scripts/claude-author.sh -p "..."`. **잔여 미구현:** `fetch_fragile`(Browser Tool v1.5)·배포 3종(deploy/invoke/teardown)·SES 발송 — 그 외 실동작·e2e 검증됨.
