...
usage: uv run python scripts/ab_prompt.py [N] [arm,arm]   # N=표본 수(기본 6) · arm 미지정=전부
      예: ab_prompt.py 4 v3.1          # v3.1 만 (이전 결과와 대조)
"""
...
# v3 시절의 _OUTPUT_CONTRACT summary 줄(f08b741) — 예산 대신 "목표 길이는 없다".
_V3_SUMMARY_LINE = (
...
def _v3_nobudget_system() -> str:
    """v3(예산 없음) 재현 — 현행에서 **예산 조각만** 옛 문구로 되돌린다.

    v3.1 의 유일한 차이가 예산임을 보이려면 대조군이 그 한 조각만 달라야 한다.
    ★ 예산은 두 곳에 있다: base md 의 '분량' 절 **+ _OUTPUT_CONTRACT 의 summary 줄**.
      둘 다 되돌려야 한다 — md 만 고치면 출력 계약에 예산이 남아 대조군이 오염된다(실제로 그랬다).
    """
    cur = _current_system()
    i, j = cur.find("### 분량"), cur.find("## skill·lens 가 정하는 것")
    if i < 0 or j < 0 or j < i:
        raise SystemExit("v3 재현 실패: 분량 절 경계를 못 찾음 — 프롬프트 구조가 바뀌었다")
    old_budget = (
        "### 분량 — 목표 길이는 없다. 담을 사실이 정한다\n"
        "- 위 우선순위의 사실을 다 담았으면 **거기서 멈춘다.** 배경·맥락·전망을 덧붙여 분량을 늘리지 마라.\n"
        "- 원문이 짧거나 세부가 빈약해 담을 사실이 적으면 **그만큼 짧게 쓴다**(1~2문장도 좋다).\n"
        "  **분량을 채우려 추측하거나 배경지식으로 메우지 마라 — 짧은 요약이 부풀린 요약보다 낫다.**\n"
        "- 반대로, 위 우선순위의 사실이 셋 이상 있는데 요약이 두 문장에 그쳤다면 **무언가를 빠뜨린 것이다.**\n"
        "  분량을 아끼려 핵심 사실을 버리지 마라.\n\n"
    )
    out = cur[:i] + old_budget + cur[j:]
    # 출력 계약의 summary 줄도 v3 판으로 교체(현행 줄은 '**3~5문장**'을 담고 있다).
    k = out.find("summary: 한국어 산문 한 문단")
    end = out.find("\nwhy_it_matters:", k)
    if k < 0 or end < 0:
        raise SystemExit("v3 재현 실패: _OUTPUT_CONTRACT 의 summary 줄을 못 찾음")
    return out[:k] + _V3_SUMMARY_LINE.rstrip("\n") + out[end:]


def _v3_nobudget_user(fs: FrozenSource, today: str) -> str:
    """v3 user turn — v3.1 과 동일(예산은 system 에만 있다)."""
...
def main() -> None:
    ...
    all_arms = {
        "v2": (_v2_system(), _v2_user),
        "v3": (_v3_nobudget_system(), _v3_nobudget_user),          # 예산 없음(대조군)
        "v3.1": (_current_system(), lambda fs, t: build_user_prompt(fs, today=t)),  # 현행
    }
    want = sys.argv[2].split(",") if len(sys.argv) > 2 else list(all_arms)
    arms = {k: v for k, v in all_arms.items() if k in want}
    if not arms:
        raise SystemExit(f"알 수 없는 팔: {want} — 가능: {list(all_arms)}")
    # 팔이 실제로 다른지 **돌리기 전에** 확인한다(오염된 A/B 를 한 번 겪었다).
