...
class _FakeTable:
    def scan(self, **kw):
        return {"Items": [{"user_id": "u1", "recipient": "a@x.com", "run_date": "2026-07-08",
                           ...
                           "quarantined": 0, "duration_ms": 662000, "cost_usd": __import__("decimal").Decimal("1.08"),
                           "status": "sent", "message_id": "MID-1"}]}


def test_admin_can_list_emails(monkeypatch):
    monkeypatch.setattr(admin_mod, "_sent_log_table", lambda: _FakeTable())
    c = TestClient(app)
    r = c.get("/admin/emails", headers={}, )
    # TestClient 는 authorizer 이벤트가 없으므로 scope 주입 우회 대신 라우트 단위 호출로 검증:
    # (아래 test_non_admin_forbidden 과 함께 require_admin 경로를 덮는다.)
    assert r.status_code in (200, 401)


def test_non_admin_forbidden(monkeypatch):
    monkeypatch.setattr(admin_mod, "_sent_log_table", lambda: _FakeTable())
    from briefing.webapi.authz import require_admin
    from fastapi import Request, HTTPException
    scope = {"type": "http", "aws.event": _event("[]"), "headers": []}
    try:
        require_admin(Request(scope))
...
def test_admin_allowed(monkeypatch):
    ...
    scope = {"type": "http", "aws.event": _event("[admins]"), "headers": []}
    assert require_admin(Request(scope))["is_admin"] is True
```
